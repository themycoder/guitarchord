...
function newArticle() {
  ...
  async function existsBySlug(slug) {
    ...
    try {
      const res = await fetch(`${API_BASE}/information/${encodeURIComponent(slug)}`);
      return res.ok;
    } catch {
      return false;
    }
...
async function loadBySlugParam(slug) {
  ...
  async function save(publish = false) {
    ...
    try {
      ...
      const url =
        method === "POST"
          ? `${API_BASE}/information`
          : `${API_BASE}/information/${encodeURIComponent(article.slug)}`;
...
function ListPanel({ onCreate, onEdit }) {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/information?${params.toString()}`);
      const json = await res.json();
  ...
  async function del() {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/information/${encodeURIComponent(slug)}`,
        { method: "DELETE" }
      );
